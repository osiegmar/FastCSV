...
///     .andThen(FieldModifiers.transform(field -> field.toUpperCase(Locale.ENGLISH)));
...
public final class FieldModifiers {
    ...
    public static final FieldModifier NOP = transform(field -> field);

    /// Modifier that modifies the field value with [String#trim()].
    /// Comments are not modified.
    public static final FieldModifier TRIM = transform(String::trim);

    /// Modifier that modifies the field value with [String#strip()].
    /// Comments are not modified.
    public static final FieldModifier STRIP = transform(String::strip);

    private FieldModifiers() {
        // Utility class
    }

    /// Builds a modifier that transforms the field value using the provided function.
    /// Comments are not modified.
    ///
    /// @param function the function to transform the field value. The contract of
    ...
    public static FieldModifier transform(final Function<? super String, String> function) {
